...
public class Level3Nav : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit hit;
        if (Physics.Raycast(RigRef.transform.position, Vector3.down, out hit, 6f, groundMask))
        {
            normal = hit.normal;
        }
        ...
        Vector3 forward = HeadRef != null ? Vector3.ProjectOnPlane(HeadRef.transform.forward, normal) : RigRef.transform.forward;
        Vector3 right = HeadRef != null ? Vector3.ProjectOnPlane(HeadRef.transform.right, normal) : RigRef.transform.right;
